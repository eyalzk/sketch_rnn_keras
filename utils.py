...
class KLWeightScheduler(Callback):
    ...
    def on_batch_begin(self, batch, logs=None):
        ...
        kl_weight = self.schedule(self.count)
        if not isinstance(kl_weight, (float, np.float32, np.float64)):
            raise ValueError('The output of the "schedule" function '
                             'should be float.')
        K.set_value(self.kl_weight, kl_weight)
        if self.verbose > 0 and self.count % 20 == 0:
            print('\nBatch %05d: KLWeightScheduler setting KL weight '
                  ' to %s.' % (self.count + 1, kl_weight))
        self.count += 1
